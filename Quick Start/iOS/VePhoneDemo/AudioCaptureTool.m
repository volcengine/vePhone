...
{
    AudioCaptureConfig *config = [[self alloc] init];
    config.channels = 2;
...
@property (nonatomic, strong, readwrite) AudioCaptureConfig *config;
@property (nonatomic, strong) dispatch_queue_t captureQueue;
@property (nonatomic, assign) BOOL isError;

@end

@implementation AudioCaptureTool

- (instancetype)initWithConfig:(AudioCaptureConfig *)config
{
    if (self = [super init]) {
        _config = config;
        _captureQueue = dispatch_queue_create("com.VePlayerDemo.AudioCapture", DISPATCH_QUEUE_SERIAL);
    }
...
- (void)start
{
    ...
    dispatch_async(_captureQueue, ^{
        ...
        if (startStatus != noErr) {
            // 捕捉并回调开始采集时的错误。
            [weakSelf callBackError: [NSError errorWithDomain: NSStringFromClass([AudioCaptureTool class]) code: startStatus userInfo: nil]];
        }
    });
}
...
- (void)stop
{
    ...
    dispatch_async(_captureQueue, ^{
        if (weakSelf.audioCaptureInstance) {
            ...
            if (stopStatus != noErr) {
                // 捕捉并回调停止采集时的错误。
                [weakSelf callBackError:[NSError errorWithDomain:NSStringFromClass([AudioCaptureTool class]) code:stopStatus userInfo:nil]];
            }
        }
    });
}
...
static OSStatus audioBufferCallBack(void *inRefCon,
    ...
    @autoreleasepool {
        AudioCaptureTool *capture = (__bridge AudioCaptureTool *)inRefCon;
        if (!capture) {
            return -1;
        }
        ...
        if (status == noErr) {
            ...
            CMSampleBufferRef sampleBuffer = [AudioCaptureTool sampleBufferFromAudioBufferList:buffers inTimeStamp:inTimeStamp inNumberFrames:inNumberFrames description:capture.audioFormat];
            // 回调数据。
            if (capture.sampleBufferOutputCallBack) {
                capture.sampleBufferOutputCallBack(sampleBuffer, inNumberFrames);
            }
            if (sampleBuffer) {
