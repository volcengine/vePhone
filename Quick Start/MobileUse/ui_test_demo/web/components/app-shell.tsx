...
export function AppShell({ children, user }: { children: ReactNode; user: User }) {
  ...
  async function submitBusinessCreate(event: FormEvent<HTMLFormElement>) {
    ...
    setBusinessError("");
    try {
      const productId = String(settingsPayload.mobile_use.product_id ?? "");
      const availability = await api.get<{ available: boolean }>(
        `/business-spaces/product-id-available?product_id=${encodeURIComponent(productId)}`,
      );
      if (!availability.available) {
        setBusinessError("Product ID 已被其它业务使用，请更换后再创建。");
        return;
      }
              ...
              <section>
                ...
                <div className="business-modal-grid">
                  <BusinessConfigInput
                    label="Product ID"
                    required
                    value={businessConfig.product_id}
                    onChange={(value) => updateBusinessConfig("product_id", value)}
                  />
                  <BusinessConfigInput
                    label="Volcengine AccountId"
...
function buildBusinessSettingsPayload(
  ...
  for (const field of ["product_id", "access_key_id", "secret_access_key", "tos_bucket", "tos_region"] as const) {
    if (!values[field].trim()) {
      return "默认执行配置必须完整：Product ID、AK/SK、TOS Bucket 和 TOS Region 都必填。";
    }
  }
  ...
  const mobileUse: NonNullable<RunnerSettingsUpdate["mobile_use"]> = {
    ...
    tos_bucket: values.tos_bucket.trim(),
    tos_region: values.tos_region.trim(),
  };
  if (values.account_id.trim()) {
    mobileUse.account_id = values.account_id.trim();
  }
