...
logger = logging.getLogger("cua_platform.schedule_runner")
    ...
    def trigger_due(
        ...
        *,
        ...
        for schedule in due_schedules:
            ...
            from cua_platform.test_plans.scheduling import compute_next_run

            next_run = compute_next_run(
                schedule.cron_expr, schedule.timezone, now
            )
            # SQLite stores naive datetimes; normalize for CAS comparison
            cas_expected = (
                expected_next.replace(tzinfo=None)
                if hasattr(expected_next, "tzinfo") and expected_next.tzinfo
                else expected_next
            )
            cas_now = now.replace(tzinfo=None) if now.tzinfo else now
            cas_next = (
