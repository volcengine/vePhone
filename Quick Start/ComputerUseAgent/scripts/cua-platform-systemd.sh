...
SERVICE_NAME="${SERVICE_NAME:-computer-use-test}"
PORT="${PORT:-8001}"
...
Environment:
  SERVICE_NAME  systemd --user service name (default: computer-use-test)
  PORT          Backend HTTP listen port (default: 8001)
...
Typical first-time setup:
  ...
  npm run build
  cp .env.example .env
  # edit .env and configure CUA credentials
...
render_unit() {
  cat <<EOF
[Unit]
Description=Computer Use Test Demo
