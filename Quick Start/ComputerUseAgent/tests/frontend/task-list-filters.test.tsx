...
it("maps stopped result filter to cancelled status query", async () => {
  const seen: Array<{
    status: string | null;
    verdict: string | null;
  }> = [];
  ...
  await screen.findByText("task_stopped");
  await user.click(screen.getByRole("combobox", { name: "执行结果筛选" }));
  await user.click(screen.getByRole("option", { name: "已停止" }));

  await waitFor(() =>
    expect(
      seen.some((entry) => entry.status === "cancelled" && entry.verdict === null),
    ).toBe(true),
  );
});
