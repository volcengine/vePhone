...
def test_scheduler_uses_account_id_snapshot_for_cua_pool():
    engine = create_engine("sqlite://")
    Base.metadata.create_all(engine)
    now = datetime(2026, 7, 29, 8, 0, tzinfo=UTC)
    try:
        with Session(engine, expire_on_commit=False) as db:
            batch = _seed_batch(
                ...
                concurrency=1,
                task_count=1,
                batch_id="cua-account",
                business_id="biz_default",
                product_id="legacy-product",
            )
            batch.config_snapshot.pop("product_id", None)
            batch.config_snapshot["account_id"] = "2103274899"
            _seed_pod(
                db,
                "i-cua-node",
                now,
            ...
            result = BatchScheduler(db).schedule(now, global_limit=1)

            assert result.task_ids == [batch.tasks[0].id]
            config = db.get(TaskRunnerConfig, batch.tasks[0].id)
            assert config is not None
            assert config.config_snapshot["account_id"] == "2103274899"
            assert config.config_snapshot["pod_id"] == "i-cua-node"
            assert "product_id" not in config.config_snapshot
    finally:
        engine.dispose()
...
def test_partial_specified_unavailability_uses_remaining_pod():
    ...
    try:
        with Session(engine, expire_on_commit=False) as db:
            ...
            _seed_pod(db, "pod-offline", now, status=4)
