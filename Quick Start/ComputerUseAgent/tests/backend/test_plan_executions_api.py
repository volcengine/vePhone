...
def test_run_single_case_plan_creates_ordered_atomic_snapshot(
    ...
    assert body["config_snapshot"]["account_id"] == "2103274899"
    assert body["config_snapshot"]["product_id"] is None
...
def test_run_plan_uses_payload_device_wait_timeout(authenticated_client):
    _configure_runner(authenticated_client)
    case_id = _create_case(authenticated_client, "等待配置")
    plan = _create_plan(authenticated_client, "等待计划", [case_id])
    payload = _run_payload(device_wait_timeout_seconds=75)
    ...
    with authenticated_client.app.state.session_factory() as db:
        ...
        assert batch.config_snapshot["device_wait_timeout_seconds"] == 75
