    ...
    *,
    ...
    with factory() as db:
        ...
        batch = TaskBatch(
            ...
            config_snapshot={"account_id": "account-refresh"},
...
async def test_refreshes_due_target_and_throttles_recent_success():
    ...
    async def refresh(business_id: str, pool_id: str) -> None:
        calls.append((business_id, pool_id))
        with factory() as db:
            db.add(PodPoolRefresh(product_id=pool_id, refreshed_at=now))
            db.commit()
    ...
    try:
        ...
        assert calls == [("biz-refresh", "account-refresh")]
...
async def test_failed_refresh_blocks_batches_and_retries_after_backoff():
    ...
    async def refresh(_business_id: str, _pool_id: str) -> None:
        nonlocal calls
        calls += 1
